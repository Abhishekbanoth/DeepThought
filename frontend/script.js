...
function fetchJSONData() {
    const cardId = [18882, 18883, 18884, 18885, 18886];
        ...
        .then((data) => {
            ...
            task.assets.forEach(element => {
                cardId.forEach(e => {
                    ...
                    if (titleElement && titleElement.id == task.task_id) {
                        titleElement.children[0].innerHTML = `<h3>${task.task_title}</h3>`;
                        document.querySelector('#panelBodyTitle').innerHTML = `${task.task_title}`;
                        titleElement.children[1].innerHTML = `<p>${task.task_description.replace(/\r\n/g, '')}</p>`;
                    }

                    if (titleElement && titleElement.id == element.asset_id) {
                        ...
                        titleElement.children[1].innerHTML = `<p> <span class="bold">Description : </span>${element.asset_description.replace(/\r\n/g, '')}</p>`;
                    }
                });
            });
        })
